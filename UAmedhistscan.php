...
if (isset($_POST[eventid]))
{
	...
	$sql = "SELECT *  from ClientEventTBL 
					where ( ClientEventTBL
					.MEDPAL = '$Medpal' and ClientEventTBL.ID = '$_POST[eventid]')";
	...
	if (!$sql_result = mysql_query($sql, $conn))
	{
		...
		$location = "Location: if_UAmedhistdetail.php";
		$severity = 1;
...
foreach($_FILES as $file_name => $file_array)
{
	...
	if ($affRows != 1)
	{
		...
		$shortmsg = "Unable to add Scaned images.";
		$severity = 1;
		LogErr($shortmsg, $errmsg, $location, $module, $severity);
	}
	...
	$sql = "INSERT INTO EventScanTBL 
				(ClientEventID, ScanID, EventScanInfo, ScanTypeID, ScanDefinitionkeyID)  
				VALUES ('$EventID', '$NewScanID', '$_POST[desc]', '$_POST[scantype]', '$_POST[scandefinition]')";
	...
	if ($affRows != 1)
	{
		...
		$location = "Location: if_UAmedhistdetail.php&eventid=";
		$shortmsg = "Unable to add detail to DB.";
		$severity = 1;
		LogErr($shortmsg, $errmsg, $location, $module, $severity);
	}
	...
	//----------------------------------------------------------------------------------------------------------
	AccessLogAdd("Medical scanned files Added.", "Ok", $module, "", $conn);
