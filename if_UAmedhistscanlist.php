...
// Set special globals.
//----------------------------------------------------------------------------------------------------------
$module = 'if_UAmedhistscanlist.php';
...
//----------------------------------------------------------------------------------------------------------
$sql = "SELECT ScanID, EventScanInfo, EventScanTBL.ScanTypeID as Scan_TypeID, ClientEventID, ScanType, URL, ScanDefinition
	from ClientEventTBL		
...
if (isset($_GET[order]))
{
	...
	switch ($_GET[order])
	{
		...
		case 'def':
			$sql .= " ORDER BY ScanDefinitionTBL.ScanDefinition ".$_SESSION[MHDef];
...
// Used to turn colors on and off 
	...
	while ($result_arr = mysql_fetch_assoc($sql_result))
	{
		...
		if ($FlipFlag == 1)
		{
			$DisplayBlock .= "
				<tr>
					<td width=\"25%\" align=center class=\"tblDetailsmTextOff\">".$result_arr[ScanType]."</td>
					<td width=\"25%\" align=center class=\"tblDetailsmTextOff\">".$result_arr[ScanDefinition]."</td>	
					<td width=\"35%\" align=center class=\"tblDetailsmTextOff\">".$result_arr[EventScanInfo]."</td>
					<td width=\"15%\" align=right class=\"tblDetailsmTextOff\">
						<a href=\"#\" onClick=\"(PopUpScanWindow('".$ExpandedURL."'))\" class=\"tblDetailsmTextLinkOff\">
							<img align=right width=20 height=20 id=\"scandetail".$bigCount."\" border=\"$DisplayBorders\" src=\"".$DisplayImage."\">
						</a>
					</td>
				</tr>
			";
		...
		else
		{
			$DisplayBlock .= "
				<tr>	
					<td width=\"25%\" align=center class=\"tblDetailsmTextOn\">".$result_arr[ScanType]."</td>
					<td width=\"25%\" align=center class=\"tblDetailsmTextOn\">".$result_arr[ScanDefinition]."</td>
					<td width=\"35%\" align=center class=\"tblDetailsmTextOn\">".$result_arr[EventScanInfo]."</td>
