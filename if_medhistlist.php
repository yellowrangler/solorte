...
if ( isset($_POST[selType]) )
{
	...
	if ($_POST[selType] == 'search')
	{
		...
		if ($_POST[Search] != "")
		{
			$sql .= " where ( (ClientProviderTBL.MEDPAL = '$Medpal' and ClientEventTBL.CurrentStatus = '1' AND ClientEventTBL.Event LIKE '%$_POST[Search]%') OR 
				(ClientProviderTBL.MEDPAL = '$Medpal' and ClientEventTBL.CurrentStatus = '1' AND EventType LIKE '%$_POST[Search]%') )
				  ORDER BY CalendarTBL.StartDate DESC";
		}
		else
		{
			//--------------------------------------------------------------------------------------------------
			...
			$sql .= "  where (ClientProviderTBL.MEDPAL = '$Medpal' and ClientEventTBL.CurrentStatus = '1') ORDER BY CalendarTBL.StartDate DESC";
		}
		// echo "sql=".$sql."\n";
	}
	else
...
{
	$sql .= " where (ClientProviderTBL.MEDPAL = '$Medpal' and ClientEventTBL.CurrentStatus = '1')";
...
{	
	...
	while ($result_arr = mysql_fetch_assoc($sql_result))
	{
		...
		if ($FlipFlag == 1)
		{
			$DisplayBlock .= "
				<tr>
					<td width=\"5%\" align=center height=17 class=\"tblDetailsmTextOff\">
						<a href=\"if_medhistdetail.php?eventid=".$result_arr[EventID]."\" target=\"detailFrame\">
							<img id=\"moredetail\" height=20 width=20 border=\"0\" src=\"images/binoculars.gif\">
						</a>
					</td>
					<td width=\"25%\" align=left height=17 class=\"tblDetailsmTextOff\">".substr($result_arr[EventType], 0, 19)."</td>
					...
					<td width=\"25%\" align=left height=17 class=\"tblDetailsmTextOff\">".$result_arr[FirstName]." ".$result_arr[LastName]." ".$result_arr[Suffix]."</td>
					";
			...
			{
				$DisplayBlock .= "
					<td width=\"5%\" align=right height=17 class=\"tblDetailsmTextOff\">
						<a href=\"if_medhistdetail.php?eventid=".$result_arr[EventID]."\"  target=\"detailFrame\" class=\"tblDetailsmTextLinkOff\">
							<img align=right id=\"scandetail\" border=\"0\" src=\"images/camera.gif\">
						</a>
					</td>
					";
			}
		...
		else
		{
			$DisplayBlock .= "
				<tr>
					...
					<td width=\"25%\" align=left height=17 class=\"tblDetailsmTextOn\">".substr($result_arr[EventType], 0, 19)."</td>
					...
					<td width=\"25%\" align=left height=17 class=\"tblDetailsmTextOn\">".$result_arr[FirstName]." ".$result_arr[LastName]." ".$result_arr[Suffix]."</td>
					";
			...
			{
				$DisplayBlock .= "
					<td width=\"5%\" align=right height=17 class=\"tblDetailsmTextOn\">
						<a href=\"if_medhistdetail.php?eventid=".$result_arr[EventID]."\" target=\"detailFrame\" class=\"tblDetailsmTextLinkOff\">
							<img align=right id=\"scandetail\" border=\"0\" src=\"images/camera.gif\">
						</a>
					</td>
