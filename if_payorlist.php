...
if (isset($_POST[Search]))
{
	...
	{
		$sqlerr = mysql_error();
		$errmsg = "$sqlerr - Error doing mysql_query join for PayorTBL with LIKE (194) sql = '$sql'";
		$location = "";
		$severity = 1;	
...
if ($countRows < 0)
{
	$errmsg = " Error less then 0 rows returned in select on PayorTBL. count = '$countRows'";
	$location = "";
	$severity = 1;	
	$shortmsg = "System DB error.  If this persists, Please call Customer Support.";
	LogErr($shortmsg, $errmsg, $location, $module, $severity);
		...
		$DisplayList .= " 
			<tr> 
				<td valign=left class=\"tblDetailsmTextOff\" id=\"td".$i."\" align=left height=20>
					<a id=\"a".$i."\" href=\"if_payorinfo.php?payorid=".$result_array[ID]."\" target=\"mainFrame\" class=\"tblDetailsmTextOff\">".$DisplayName."</a>
				</td> 
			</tr>
			";
		...
		$DisplayList .= " 
			<tr> 
				<td valign=left class=\"tblDetailsmTextOn\" id=\"td".$i."\" align=left height=20>
					<a id=\"a".$i."\" href=\"if_payorinfo.php?payorid=".$result_array[ID]."\" target=\"mainFrame\" class=\"tblDetailsmTextOn\">".$DisplayName."</a>
				</td> 
			</tr>
			";
